<?php
if($_POST['config'] == 'config.py') {

	$filename='/opt/phoneyc-SjtuISADTeam/config.py';
	chmod ($filename, "0777");
...
if(isset($_POST['builderDataWindow'])) {
	...
	if($_POST['confName'] == 'config.py') {
		$confName=$_POST['confName'];
	 	$myFile = '/opt/phoneyc-SjtuISADTeam/config.py';
		...
		<div id="wrapper">
  	 		<div id="branding">
   				...
   				<ul class="topNav">
					...
					<li><a href="phoneyc_config.html" class="topNavAct">PhoneyC</a></li>
   			...
   			<div id="contant">
					...
					<div class="builderFormMenu">
						...
						<select name="config">
							<option value="config.py">&nbsp; PhoneyC Main Config &nbsp;</option>
						</select>
						<input name="submit" value="" type="submit" class="builderSubmit"/>
						</form>
					</div>
