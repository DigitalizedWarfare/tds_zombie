<?php
if($_POST['config'] == 'config.sample') {

	$filename='/opt/Honeyd/config.sample';
	chmod ($filename, "0777");
...
if(isset($_POST['builderDataWindow'])) {
	...
	if($_POST['confName'] == 'config.sample') {
		$confName=$_POST['confName'];
	 	$myFile = '/opt/Honeyd/config.sample';
		...
		<div id="wrapper">
  	 		<div id="branding">
   				...
   				<ul class="topNav">
					...
					<li><a href="honeyd_config.html" class="topNavAct">HoneyD</a></li>
   			...
   			<div id="contant">
					...
					<div class="builderFormMenu">
						...
						<select name="config">
							<option value="config.sample">&nbsp; HoneyD Main Config &nbsp;</option>
						</select>
						<input name="submit" value="" type="submit" class="builderSubmit"/>
						</form>
					</div>
